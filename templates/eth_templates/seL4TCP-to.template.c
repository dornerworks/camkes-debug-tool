...
void */*? me.to_interface.name?*/_send_buf_buf(unsigned int id);
void */*? me.to_interface.name?*/_recv_buf_buf(unsigned int id);
...
static void tcp_tmr_callback(void)
{
    ...
    timer_irq_reg_callback((void *)tcp_tmr_callback, NULL);
}
...
void /*? me.to_interface.name ?*/__run(void)
{
    ...
    while (1)
    {
        ...
        if(tcp_call_type == TCP_SEND)
        {
            ...
            {
                void * buffer = /*? me.to_interface.name?*/_send_buf_buf(badge);
        ...
        else
        {
            ...
            {
                void *p = /*? me.to_interface.name ?*/_recv_buf_buf(badge);
