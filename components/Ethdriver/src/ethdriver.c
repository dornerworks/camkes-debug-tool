 ...
 */
static void irq_handle(void *cookie)
{
    int UNUSED err;
    err = ethdriver_lock();
    eth_driver.i_fn.raw_handleIRQ(&eth_driver, 0);
    err = irq_reg_callback(irq_handle, NULL);
    err = ethdriver_unlock();
}
...
#ifdef CONFIG_PLAT_IMX6
    error = ethif_imx6_init(&eth_driver, ioops, NULL);
#else
    error = ethif_zynq7000_init(&eth_driver, ioops, (void *)EthDriver);
#endif
    assert(!error);

    done_init = 1;

    error = irq_reg_callback(irq_handle, NULL);
